...
ultimate_epoch_d = sys.argv[2].strip()
...
##source data
teamfile = sys.argv[1]
boutfiles = ["./flattrackstats_bouts_2018-05-09.csv",] 
tournamentfile = "./flattrackstats_tournaments_2018-05-09.csv"
###sequencing
##initialisation
teams_in = pfc.import_teamvector(teamfile)
...
#print len(bouts_in)
##selection
print "Currently, {0} teams".format(len(teams_in))
print "Sample team 0: {0}".format(teams_in[teams_in.keys()[0]])

teams_winnowed = pfc.select_teamvector(teams_in,genuses=['W','M','C','J'])
...
for date_i in range(1,len(dates)):
	...
	print "Bouts: {0}".format(len(bouts_in))	
	print "Teams: {0}".format(len(teams_winnowed))
	...
	flag = True #only do confidences if we're on the end run, which is printed
	for sg in subgraphs:
	...
	#and so "score" = s1*hadv - s2 -> if we shift by S, score = (s1-S)*hadv - (s2-S) = s1*hadv-s2 - S(hadv-1) 
		# need to adjust this to tag on the "most recent bout" for each team, so we can measure "staleness" ? [or is this close to our original sum cliquity?]
	...
	if (end_time):
		...
		f=open('{0}.out'.format(sys.argv[2]),'w')
		grp=0 #group id
			...
			for i in group:
				if grp == 0:
					...
					try:
						...
						info = sg0_cliques[current_teams[i[0]]] #split team, so need the current version of it, which will be what sg0_cliques knows
					clique_id = info['localpart']
					cliquity = int(10*info['local']/info['global'])/10.0 #this is the (rounded to 1 dp) ratio of the freshness strength of the team's links within their clique wrt outside their clique (should be always < 1?)
				else:
					clique_id = 0
					cliquity = 0
					info = { 'local':1, 'global':1, 'maxlink':1 } #dummy data 
				string = "{0}|{1:d}|{2:d}|{3:.3f}|{4:.2f}|{5:.4f}|{6:.2f}|{7:.2f}|{8:.2f}|{9:d}".format(i[0],grp, clique_id,i[1],i[2],i[3],info['local'],info['global'],info['maxlink'],counter) #note - this is a change to the format (cliquity split to clique-local and global capacities, and added max_links as a recency measure) which moves the counter by 1 position right
				#
				#			Observation: maxlink is [0,1]. maxlink < 0.33 or so implies significant staleness, and should be flagged in a display engine
				#
